...
def podspec_source_build_from_github_stable_branch()
    # [macOS
    # React Native macOS lags React Native, so use the stable-branch commit at
    # the React Native merge base instead of the latest potentially incompatible commit.
    tuple = hermes_commit_at_merge_base(HERMES_STABLE_BRANCH)
    commit = tuple[:commit]
    timestamp = tuple[:timestamp]
    hermes_log("Using Hermes commit from #{HERMES_STABLE_BRANCH} at the merge base with facebook/react-native: #{commit} and timestamp: #{timestamp}")
    return {:git => HERMES_GITHUB_URL, :commit => commit}
    # macOS]
end
