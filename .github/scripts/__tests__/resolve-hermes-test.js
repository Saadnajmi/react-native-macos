...
test.each([
  [undefined, metadata.HERMES_VERSION_NAME, 'HERMES_VERSION_NAME', 'Debug'],
  ['', metadata.HERMES_VERSION_NAME, 'HERMES_VERSION_NAME', 'Debug'],
  ['true', metadata.HERMES_VERSION_NAME, 'HERMES_VERSION_NAME', 'Release'],
  ['0', metadata.HERMES_VERSION_NAME, 'HERMES_VERSION_NAME', 'Debug'],
  ['1', metadata.HERMES_VERSION_NAME, 'HERMES_VERSION_NAME', 'Release'],
])(
...
test('CI concrete runtime metadata retains the independent checked-in source tag', () => {
  const tag = fs
    .readFileSync(
  ...
  expect(result.stdout).toContain(
    `Using HERMES_VERSION_NAME=${metadata.HERMES_VERSION_NAME}`,
  );
...
test.each([undefined, '', 'true', '0', '1'])(
  ...
  flag => {
    ...
    expect(resolve.stdout).not.toContain('Resolved Hermes tag:');
